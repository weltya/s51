...
def lambda_handler(event, context):
    try:
        ...
        response = table.scan(
            FilterExpression=Attr('clientEmail').eq(client_email)
        )
        ...
        for item in response.get('Items', []):
            reservation_info = {
                ...
                'restaurantEmail': item.get('restaurantEmail', 'N/A'),
                'restaurantPhone': item.get('restaurantPhone', 'N/A'),
                'restaurantName': item.get('restaurantName', 'N/A')
            }
